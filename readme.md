...
# The Albert Bar & Restaurant Website
## User Value
...
As a potential diner, I want a visual booking grid so that I can see real-time table availability and select intuitively.
As an allergy-conscious guest, I want 3D dish spins with side-panel allergen info so that I can assess options confidently.
...
Project Board
Managed via Trello/GitHub Projects:

Columns: Backlog, To Do, In Progress, Review, Done.
Labels: Feature, Bug, UX, Backend.
Cards: Linked to user stories, with acceptance criteria (e.g., "3D spin loads <2s").
...
Skeleton (Wireframes)
Wireframes (attached PDF/image) outline layouts:

Home: Hero image with CTAs; feature cards; footer.
Booking: Calendar grid with color-coded tables.
Menu: Tabbed categories; modal for 3D viewer + allergens.
...
Tables:
reservations: id (PK), user_email, date_time, party_size, table_id (FK), status (pending/confirmed).
menu_items: id (PK), name, description, price, category, allergens (JSON array), model_url (for 3D GLTF).
orders: id (PK), reservation_id (FK), item_id (FK), quantity, notes.
users: id (PK), email, preferences (JSON for allergies).

Relationships: One-to-many (Reservation → Orders); Indexes on date_time for queries.
...
Architecture: MVT pattern. Apps: core (views/templates), api (DRF for JS fetches), booking, menu. URLs: /api/reservations/, /api/menu/. Middleware for CSRF; Celery for email confirmations.
...
Placeholders: SVG mocks in /static/img/ (e.g., dish-spin-placeholder.svg); grayscale filters for elegance.
Real Assets: Pending scans/photos—restaurant facade (attached image: brick building with blue sign), dish 3D GLTFs.
...
Browsers: Chrome, Firefox, Safari, Edge (latest +1).
...
Grok (xAI): Generated initial wireframe analysis, user stories, and model schemas; iterated on design prompts.
Ethical: AI outputs reviewed for accuracy; no direct user data used.
...
Development Team: Hypothetical contributors for JS/Django.
